...
def start_import(instance: str) -> str:
    ...
    LOG_NAME = f"{controller.APP_NAME}_{instance}"

    default_item_mapping: dict = next((d for d in get_table_mapping() if d.get("type") == "Item"), None)

    if not default_item_mapping:
        make_log(f"Could not get the default table mapping for Item", "ERROR", LOG_NAME)
    ...
    try:
        ...
        if not selectline_items:
            make_log(f"Could not fetch any data", "ERROR", LOG_NAME)
        ...
        for fetched_item in selectline_items:
            ...
            if not selectline_id:
                make_log(f"Invalid selectline id", "ERROR", LOG_NAME)
                invalid_selectline_id = True
                continue
 
            if controller.check_mapping_exists(f"{default_item_mapping['table_name']}:{fetched_item[default_item_mapping['primary_key']]}"):
                print("mapping exists") #TODO Update...
            else:
                frappe.enqueue(
        ...
        make_log(f"Error on fetching items: {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
...
def create_item_and_mapping(fetched_item: dict, selectline_id: str, instance: str) -> None:
    ...
    if frappe.db.exists("Item", fetched_item["Artikelnummer"]):
        make_log(f"Item with name '{fetched_item["Artikelnummer"]}' already exists", "ERROR", LOG_NAME)
        return
    ...
    try:
        ...
        try:
            ...
            make_log(f"Could not update item taxes for '{new_item.item_code}': {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
        ...
        try:
           fetch_and_create_item_prices(instance=instance, item_code=new_item.item_code)
            ...
            make_log(f"Could not create Item Prices for '{fetched_item["Artikelnummer"]}': {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)

    except Exception as e:
        make_log(f"Could not create Item '{fetched_item["Artikelnummer"]}': {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
        return
    ...
    try:
        new_item_mapping: str = controller.create_mapping(
            ...
            selectline_id=selectline_id,
            mapping=get_item_mapping_schema(docname=new_item.item_code, item_group=new_item.item_group, uom=new_item.stock_uom)
        )

        make_log(f"Item '{new_item.item_code}' successfully created and mapped in '{new_item_mapping}'", "INFO", LOG_NAME)

    except Exception as e:
        make_log(f"Could not create mapping for item '{fetched_item["Artikelnummer"]}': {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
        return
...
def create_item_group(sl_item_group: str) -> str | None:
    ...
    try:
        ...
        make_log(f"Item group '{sl_item_group}' successfully created", "INFO", LOG_NAME)
        return new_item_group.item_group_name
    
    except Exception as e:
        make_log(f"Could not create item group '{sl_item_group}': {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
        return None
...
def create_uom(sl_uom) -> str:
    ...
    try:
        ...
        make_log(f"UOM '{sl_uom}' successfully created", "INFO", LOG_NAME)
        return new_uom.uom_name
    
    except Exception as e:
        make_log(f"Could not create UOM '{sl_uom}': {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
        return None
...
def fetch_and_create_item_prices(instance: str, item_code: str) -> None:
    ...
    try:
        ...
        if sl_item_ek_prices:
            for price in sl_item_ek_prices:
                ...
                try:
                    ...
                    make_log(f"Buying Item Price for Item {item_code} with amount {fetched_ek} successfully created", "INFO", LOG_NAME)
                    ...
                    make_log(f"Buying Item Price mapping {new_item_price_mapping} for Item {item_code} successfully created", "INFO", LOG_NAME)

                except Exception as e:
                    make_log(f"Could not create Item Price for Item {item_code}: {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
                    continue
        ...
        if sl_item_vk_prices:
            for price in sl_item_vk_prices:
                ...
                try:
                    ...
                    make_log(f"Selling Item Price for Item {item_code} with amount {fetched_vk} successfully created", "INFO", LOG_NAME)
        ...
        make_log(f"Could not create Item Price: {e} {frappe.get_traceback()}", "ERROR", LOG_NAME)
        return None
...
def get_item_mapping_schema(docname: str, item_group: str, uom: str) -> list:
    return [
        {"mapping_doctype": "Item", "docname": docname, "fieldname": "name", "selectline_column": "Artikelnummer"},
        {"mapping_doctype": "Item", "docname": docname, "fieldname": "description", "selectline_column": "Bezeichnung"},
...
def get_table_mapping() -> list:
    ...
    if not default_table_mapping:
        make_log(f"Could not get the default table mapping", "ERROR", LOG_NAME)
